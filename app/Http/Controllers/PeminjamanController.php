...
{
    $buku = Buku::with(['ulasan.user'])->get();
...
public function kembalikan(Request $request, $id) {
    ...
    return redirect()->route('riwayat')->with('success', 'Permintaan pengembalian dikirim. Silakan serahkan buku ke petugas.');
}


public function konfirmasiIndex() {
    $peminjaman = Peminjaman::with(['buku.ulasan.user', 'user'])
...
public function tambahKoleksi(Request $request)
{
    ...
    return back()->with('info', 'Buku ini sudah ada di dalam koleksi Anda.');
}
public function daftarKoleksi()
{
    $koleksi = \DB::table('koleksi_pribadi')
                ...
                ->select('koleksi_pribadi.KoleksiID', 'buku.*') 
                ->get();
...
public function balasUlasan(Request $request, $id)
{
    ...
    \DB::table('ulasanbuku')
        ->where('UlasanID', $id)
        ->update(['BalasanAdmin' => $request->BalasanAdmin, 'updated_at' => now()]);
