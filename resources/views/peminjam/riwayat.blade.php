...
<td>
    ...
    @if($p->StatusPeminjaman == 'Dipinjam')
         <form action="{{ route('kembalikan', $p->PeminjamanID) }}" method="POST" class="d-inline">
            @csrf
            <button type="submit" class="btn btn-warning btn-sm rounded-pill px-3 shadow-sm text-white fw-bold" onclick="return confirm('Apakah Anda yakin ingin mengembalikan buku ini?')">
...
<form action="{{ route('kembalikan', $p->PeminjamanID) }}" method="POST">                    @csrf
